...
exclude <- rownames(pheno)[which(is.na(age))] # "GSM2334015" "GSM2334342" "GSM2334619"

# Bad Samples/probes
rgSet <- read.metharray.exp(getwd(), extended = T)
qc <- ENmix::QCinfo(rgSet)
qc$badsample # "GSM2334261_9379082108_R01C02" "GSM2334328_9379082151_R01C02"
indices <- match(qc$badsample, colnames(rgSet))
rgSet <- rgSet[,-indices] # erase bad samples and redo QC
qc <- ENmix::QCinfo(rgSet)
...
pheno$`gender:ch1` <- as.factor(pheno$`gender:ch1`)  
levels(pheno$`gender:ch1`) <- c('F', 'M')
...
# Prepare batch
batch <- unlist(lapply(X = strsplit(colnames(rgSet), split = '_'), FUN = function(X) {X[2]}))
length(table(batch)) # 62 batches
batch <- as.factor(batch)
levels(batch) <- as.character(1:length(unique(batch)))
...
rgSamples <- unlist(lapply(X = strsplit(colnames(nocombat.beta.sqn), split = '_'), function(X) { paste(X[[1]])}))
length(rgSamples) # 727
pheno <- pheno[rgSamples,]
age <- as.numeric(pheno$`age:ch1`)
gender <- as.factor(pheno$`gender:ch1`)

df <- as.data.frame(t(nocombat.beta.sqn))
df$age <- age
df$gender <- gender
modcombat <- model.matrix(~age+as.factor(gender), data=df)
rm(df) ; gc()
...
sum(beta.sqn < 0) # 1041
sum(beta.sqn > 1) # 113
sum(is.na(beta.sqn)) # 0
dim(beta.sqn) # 346469    727
########## QC ###############
